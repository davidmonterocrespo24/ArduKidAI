...
TOOLS: dict[str, ToolSpec] = {
    ...
    "show_tutor_panel": ToolSpec(
        ...
        description=(
            ...
            "- checklist: {kind:'checklist', title?, intro?, items:[string,...], done_label?} - "
            "a follow-along wiring checklist; the child ticks each connection and the 'done' "
            "button stays disabled until all are ticked. Use the real pins/parts (call "
            "describe_circuit). When they finish, you get a turn to congratulate them.\n"
            ...
            "predict-then-run: the child predicts the outcome, then a 'Run it' button runs the "
            "real simulation. Use BEFORE running so they reason about the code first.\n"
            ...
            "- flashcards: {kind:'flashcards', title?, cards:[{front, back, part?}]} - flip cards "
            "for review; 'part' optionally shows a real component (catalog type id) on the front.\n"
            "- progress: {kind:'progress', title?, skills:[{label, status}], badges?:[{label, "
            "earned}]} - a skill map (status: done|doing|locked) + badges, to celebrate growth. "
