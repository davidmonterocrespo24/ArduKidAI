...
async def run_chat(*, session: SessionState, user_message: str) -> AsyncIterator[SSEEvent]:
    ...
    try:
        ...
        async for event in client.chat(session=session, user_message=user_message):
            yield event
